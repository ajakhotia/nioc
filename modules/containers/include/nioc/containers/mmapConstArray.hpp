...
public:
  ...
  explicit MmapConstArray(std::filesystem::path path):
    mFile{File::open(std::move(path))},
    mMapping{Mapping::readOnly(mFile, mFile.size())}
  {
    if(mMapping.size() % sizeof(ValueType) != 0)
    {
